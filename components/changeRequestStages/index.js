...
import { BiChevronLeftCircle, BiChevronRightCircle, BiRightArrow } from "react-icons/bi";
...
function ChangeRequestStages(props) {
    ...
    const processStageList = [...new Set(processStageData.map(item => item["Current Process Stage"]))];
    ...
    const leftBtnIcon = (
        <span onClick={handleClickLeft} className={styles.leftBtn} disabled={currentIndex <= 0}>
            <BiChevronLeftCircle style={{ color: currentIndex <= 0 ? "gray" : "green" }} />
        </span>
    );

    const rightBtnIcon = (
        <span onClick={handleClickRight} className={styles.rightBtn} disabled={currentIndex >= (processStageList.length - 1)}>
            <BiChevronRightCircle style={{ color: currentIndex >= (processStageList.length - 1) ? "gray" : "green" }} />
        </span>
    );
        ...
        <div>
            ...
            <div className={styles.changeRqstList}>
                {currentProcessStgItems.map((name, index) => (
                    <a href={name["Link to CP page"]} target="_blank" rel="noreferrer">
                        <BiRightArrow />
