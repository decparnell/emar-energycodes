...
const DefinitionTables = (props) => {
  ...
  */
  ...
  const fetchDefinitionsByLetter = async (letter) => {
    ...
    let url = `https://prod-31.uksouth.logic.azure.com/workflows/87f5aaaab030422a86899a6ca24c1351/triggers/request/paths/invoke/documentId/2/version/${props.version}/sectionId/2237/letter/${letter}?api-version=2016-10-01&sp=%2Ftriggers%2Frequest%2Frun&sv=1.0&sig=eWSPnGwLkUOb6ZM2MLxynlkR1vR09rab9cfCJNeAefA`;
    setError(null);
  ...
  const CreateAlphabeticTableContent = (props) => {
    ...
    formatData.map((section) => {
      ...
      content.push(
        <div id={`sec${section.sectionId}`} className={styles.tableDefinitions}>
          <h3>
  ...
  const DefinitionsNotFound = () => {
    return (
      <div className={styles.infoLabelDefinitions}>
        <p>
          Definitions are not found for the letter:{" "}
          <b>{selectedAlphabeticLetter}</b>
        </p>
      </div>
    );
  };
  ...
  return (
    <>
      <p className={styles.infoLabelDefinitions}>
        Definitions are organized alphabetically. Please select a letter.
      </p>
      <table
        className={styles.alphabetIndexTable}
        id={
          selectedAlphabeticLetter === "" || data.length <= 2
            ? `sec${defaultSectionId.sectionId}`
            : "AlphabeticTableSearchId"
        }
        key={
          selectedAlphabeticLetter === ""
            ? defaultSectionId.sectionName
            : "AlphabeticTableSearchKey"
        }
      >
        <tbody>
              ...
              {row.map((letter, columnIndex) => (
                <td
                  ...
                  className={`${styles.cellHovered} ${
                    selectedAlphabeticLetter === letter
                      ? styles.cellSelected
                      : ""
                  }`}
                >
